...
class MJXBaseEnv:
    ...
    def __init__(
        ...
        with Progress(
            ...
            progress.update(load_task, advance=1, description=f"[cyan]解析路径: {xml_path.name}")
    ...
    def _resolve_xml_path(self, xml_path: str) -> Path:
        ...
        if not path.is_absolute():
            ...
            candidates = [
                ...
                Path(__file__).parent.parent.parent.parent / "assets" / xml_path,  # 项目assets目录
    ...
    def _display_model_info(self):
        ...
        console.print(table)
        console.print(f"[dim]执行器: {', '.join(self.actuator_names[:5])}{'...' if len(self.actuator_names) > 5 else ''}[/dim]")
